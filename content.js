...
(() => {
  ...
  const SENSITIVE_RE = /급여|명세서|연말정산|원천징수|건강검진\s*결과/;
  const PATIENT_ID_RE = /\b\d{8}\b|\b(19[0-9]{2}|200[0-9])[-.\/]\d{1,2}[-.\/]\d{1,2}\b/;
  function isPatientRelated(info) {
    if (info.senderEmails.some(e => PATIENT_SENDERS.some(p => p.startsWith('@') ? e.endsWith(p) : e === p))) return true;
    if (PATIENT_RE.test(info.subject) || PATIENT_RE.test(info.snippet)) return true;
    if (PATIENT_ID_RE.test(info.subject) || PATIENT_ID_RE.test(info.snippet)) return true;
    if (SENSITIVE_RE.test(info.subject)) return true;
    return false;
  }
  function maskIds(t) {
    return t
...
.jev-done{outline:2px dashed #188038;outline-offset:-2px}.jev-wait{background:#e8f0fe;color:#1a73e8}.jev-err{background:#fce8e6;color:#c5221f}`;
  ...
  function render(info, data, state) {
    ...
    if (data && data.excluded) { b.className = 'jev-badge jev-min'; b.textContent = '🔒'; b.title = '환자 관련 또는 민감 메일로 판단: 본문을 읽지 않았고 Jev에 보내지 않음'; return; }
  ...
  async function runJob({ info }) {
    try {
      ...
      if (text && (PATIENT_ID_RE.test(text) && /등록번호|환자|병동|진단/.test(text))) {
        // body turned out to contain patient identifiers: do not send
        memCache[info.key] = { excluded: true, t: Date.now() };
  ...
  async function scan() {
    ...
    for (const info of infos) {
      if (isPatientRelated(info)) { render(info, { excluded: true }); continue; }
