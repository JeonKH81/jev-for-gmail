...
- **환자 관련 메일은 전송하지 않습니다.** 발신자(설정에서 지정), 제목·미리보기·본문의 환자 관련 키워드(환자, 시술, 입원, 등록번호, 대상자, SAE 등)와 환자 ID 패턴으로 걸러냅니다. 제목이나 미리보기에서 감지되면 본문도 가져오지 않습니다. 급여 명세서 같은 민감 메일도 제외합니다.
- **내 개인정보가 담긴 메일도 전송하지 않습니다.** 제목·미리보기에 주민번호, 계좌, 입금·송금, 여권, 카드번호, 비밀번호, 인증번호, 주소지·배송지, 등본, 급여명세서 같은 표현이 있으면 본문을 가져오지 않습니다. 제목에 드러나지 않는 경우에는 브라우저 안에서만 본문을 확인해 주민번호·카드번호·계좌번호·주소·여권번호가 있으면 전송을 중단합니다 (이 확인은 외부로 나가지 않습니다).
