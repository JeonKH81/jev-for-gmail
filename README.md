...
- Gmail thread의 `From`, `To`, `Cc`, `Bcc`, `Reply-To` 및 인쇄 보기의 이름·이메일 발신자 줄은 브라우저 안에서 `[SELF]` 또는 `[REDACTED]`로 치환합니다. 본문과 제목의 이메일, 전화번호, 생년월일, 주민번호, 카드번호, 계좌·여권·환자 ID, 주소(번지 없는 주소 줄 포함), 비밀번호·ID 값도 가립니다. 외부 이름 탐지 API는 사용하지 않습니다.
